...
export default class ContractorProfile extends React.Component {
  ...
  CardExampleLinkCard = bid => {
    return (
      <Card
        ...
        meta={
          !bid.task.task_done
            ? `Current lowest Bid: $${this.sortBids(bid.task.bids).price}`
            : null
        }
        description={
          !bid.status
            ? `Your Bid: $${bid.price}`
            : `You Won With A Bid Of: $${bid.price}!`
        }
      />
    );
  };
  ...
  textMe = () => {
    ...
    fetch("https://best-offer-backend.herokuapp.com/messages/reply", {
      method: "POST",
      headers: {
  ...
  render() {
    return (
      <div>
        ...
        <h1>
          {" "}
          Hi, {this.props.contractor ? this.props.contractor.name : null}
        </h1>
